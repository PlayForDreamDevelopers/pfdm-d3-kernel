...
bool enable_dump = IS_ENABLED(CONFIG_POWER_RESET_QCOM_DOWNLOAD_MODE_DEFAULT);
bool con_enabled = IS_ENABLED(CONFIG_SERIAL_MSM_GENI_CONSOLE_DEFAULT_ENABLED);
int __init devmode_init(char *s)
{
       if (s == NULL || s[0] == '\0') {
               return -1;
       }
...
__setup("androidboot.dev_disabled=", devmode_init);
EXPORT_SYMBOL(enable_dump);
EXPORT_SYMBOL(con_enabled);
