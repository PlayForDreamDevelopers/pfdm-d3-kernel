	...
	const struct adreno_hwsched_ops *target_hwsched_ops)
{
	...
	int i;
	int status = 0;
	struct sched_param scheduler_settings = { .sched_priority = 90 };
...
#else
	status = sched_setscheduler(hwsched->worker->task, SCHED_FIFO, &scheduler_settings);
	if (status) {
		dev_err(device->dev, "Failed to set SCHED_FIFO. (%d)\n", status);
	}
#endif
