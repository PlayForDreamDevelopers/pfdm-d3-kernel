...
#ifdef CONFIG_PRODUCT_YVR_D3_EVT2
	/* aw35711 and aw35742 */
	struct gpio_desc *cc_dir_gpio;
	struct regulator *aw35711_35742_vcc33;
	struct pinctrl *aw35711_35742_pinctrl;
	struct pinctrl_state *aw35711_enable;
	struct pinctrl_state *aw35711_disable;
	struct pinctrl_state *aw35711_cc1;
	struct pinctrl_state *aw35711_cc2;
	struct pinctrl_state *aw35742_lt7911d;
	struct pinctrl_state *aw35742_typec;
#endif
};
...
static uint32_t    usb_orientation = 0;

static ssize_t usb_orientation_show(struct kobject* kobj, struct kobj_attribute *attr, char *buf) {
	return sprintf(buf, "%d\n", usb_orientation ? 1 : 2);
}

static ssize_t usb_orientation_store(struct kobject *kobj, struct kobj_attribute *attr,const char *buf, size_t count) {
	return count;
}

static struct kobj_attribute usb_orientation_attr =
  	__ATTR(usb_orientation, 0644,
  	       usb_orientation_show,
  	       usb_orientation_store);
...
#ifdef CONFIG_PRODUCT_YVR_D3_EVT2
static void dwc3_msm_update_aw35711_orientation(struct dwc3_msm *mdwc,int orientation);
#endif
static void dwc3_set_ssphy_orientation_flag(struct dwc3_msm *mdwc)
{
	...
	if (mdwc->orientation_override) {
		...
		usb_orientation = orientation;

		#ifdef CONFIG_PRODUCT_YVR_D3_EVT2
		/* update aw35711 orientation */
		dwc3_msm_update_aw35711_orientation(mdwc,orientation);
...
static void dwc3_msm_update_aw35711_orientation(struct dwc3_msm *mdwc,int orientation)
{
	...
	if (orientation == 0 && mdwc->aw35711_cc1)
		ret = pinctrl_select_state(mdwc->aw35711_35742_pinctrl, mdwc->aw35711_cc1);
	else if (orientation != 0 && mdwc->aw35711_cc2)
		ret = pinctrl_select_state(mdwc->aw35711_35742_pinctrl, mdwc->aw35711_cc2);

	if (!ret)
		dev_info(mdwc->dev, "aw35711 orientation is set to %d\n", orientation);
	else
		dev_err(mdwc->dev, "failed to set aw35711 orientation to %d\n", orientation);
}

static void dwc3_msm_init_aw35711_35742(struct platform_device *pdev)
{
	struct dwc3_msm *mdwc = platform_get_drvdata(pdev);
	int ret;
	...
	mdwc->cc_dir_gpio = devm_gpiod_get_optional(&pdev->dev, "qcom,cc-dir", GPIOD_IN);
	if (!mdwc->cc_dir_gpio)
		dev_err(&pdev->dev, "failed to get CC dir gpio\n");

	/* vcc33 regulator */
	mdwc->aw35711_35742_vcc33 = devm_regulator_get(&pdev->dev, "aw35711-3p3");
        if (!IS_ERR_OR_NULL(mdwc->aw35711_35742_vcc33)) {
		ret = regulator_set_voltage(mdwc->aw35711_35742_vcc33, 3300000, 3300000);
		if (ret)
			dev_err(&pdev->dev, "failed to set voltage for aw35711 & 35742 power supply\n");

		ret = regulator_enable(mdwc->aw35711_35742_vcc33);
		if (ret) {
			dev_err(&pdev->dev, "failed to enable aw35711 & 35742 power supply\n");
			return;
		}
	} else {
		dev_err(&pdev->dev, "failed to get aw35711 & 35742 power supply\n");
		mdwc->aw35711_35742_vcc33 = NULL;
		return;
	}
#endif

	/* pinctrl */
	mdwc->aw35711_35742_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (!IS_ERR_OR_NULL(mdwc->aw35711_35742_pinctrl)) {
		/* aw35711 enable */
		mdwc->aw35711_enable = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35711_enable");
		if (IS_ERR_OR_NULL(mdwc->aw35711_enable)) {
			dev_err(&pdev->dev, "failed to find aw35711_enable pinctrl state\n");
			mdwc->aw35711_enable = NULL;
		}

		/* aw35711 enable */
		mdwc->aw35711_disable = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35711_disable");
		if (IS_ERR_OR_NULL(mdwc->aw35711_disable)) {
			dev_err(&pdev->dev, "failed to find aw35711_disable pinctrl state\n");
			mdwc->aw35711_enable = NULL;
		}

		/* aw35711 cc1 */
		mdwc->aw35711_cc1 = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35711_dir_cc1");
		if (IS_ERR_OR_NULL(mdwc->aw35711_cc1)) {
			dev_err(&pdev->dev, "failed to find aw35711_dir_cc1 pinctrl state\n");
			mdwc->aw35711_cc1 = NULL;
		}

		/* aw35711 cc2 */
		mdwc->aw35711_cc2 = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35711_dir_cc2");
		if (IS_ERR_OR_NULL(mdwc->aw35711_cc2)) {
			dev_err(&pdev->dev, "failed to find aw35711_dir_cc2 pinctrl state\n");
			mdwc->aw35711_cc2 = NULL;
		}

		/* aw35742 aux to lt7911d */
		mdwc->aw35742_lt7911d = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35742_aux_lt7911d");
		if (IS_ERR_OR_NULL(mdwc->aw35742_lt7911d)) {
			dev_err(&pdev->dev, "failed to find aw35742_aux_lt7911d pinctrl state\n");
			mdwc->aw35742_lt7911d = NULL;
		}

		/* aw35742 aux to typec */
		mdwc->aw35742_typec = pinctrl_lookup_state(mdwc->aw35711_35742_pinctrl, "aw35742_aux_typec");
		if (IS_ERR_OR_NULL(mdwc->aw35742_typec)) {
			dev_err(&pdev->dev, "failed to find aw35742_aux_typec pinctrl state\n");
			mdwc->aw35742_typec = NULL;
		}
	} else {
	...
	/* set aw35711 & aw35742 state */
	if (mdwc->aw35711_enable) {
		ret = pinctrl_select_state(mdwc->aw35711_35742_pinctrl, mdwc->aw35711_enable);
		if (ret)
			dev_err(&pdev->dev, "failed to enable aw35711\n");
	}

	if (mdwc->aw35742_typec) {
		ret = pinctrl_select_state(mdwc->aw35711_35742_pinctrl, mdwc->aw35742_typec);
		if (ret)
			dev_err(&pdev->dev, "failed to set aw35742 to connect AUX and typec connector\n");
	}
}
#endif
...
#ifdef CONFIG_PRODUCT_YVR_D3_EVT2
	dwc3_msm_init_aw35711_35742(pdev);
#endif

	pKobj =  kobject_create_and_add("usb", &mdwc->dev->kobj/*kernel_kobj->parent*/);
	if (!pKobj) {
		dev_err(mdwc->dev, "usb orientation kobject create failed\n");
	} else {
		ret = sysfs_create_file(pKobj, &usb_orientation_attr.attr);
		if(ret){
			dev_err(mdwc->dev, "usb orientation sysfs create failed\n");
		}else{
			dev_err(mdwc->dev, "usb orientation sysfs create success\n");
		}
	}
	return 0;
