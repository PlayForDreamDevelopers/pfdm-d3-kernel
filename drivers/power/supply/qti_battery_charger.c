...
static const int battery_prop_map[BATT_PROP_MAX] = {
	...
	[BATT_DISABLE_CHARGE] = POWER_SUPPLY_PROP_DISABLE_CHARGE,
	[BATT_ICHG_LIMIT_STATUS] = POWER_SUPPLY_PROP_ICHG_LIMIT_STATUS,
};
...
static bool use_fake_soc = false;
...
static int battery_psy_set_ichg_limit_status(struct battery_chg_dev *bcdev, int val)
{
	int rc =0;
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_ICHG_LIMIT_STATUS, val);
	if (rc < 0) {
		pr_err("Failed to set ichg_limit_status\n");
	} else {
		pr_info("set ichg_limt_status:%d\n",val);
	}
	return rc;
}
static int battery_psy_disable_charge(struct battery_chg_dev *bcdev,
					int val)
{
	int rc = 0;
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_DISABLE_CHARGE, val);
	if (rc < 0) {
		pr_err("Failed to %s charge, rc=%d\n", val ? "disable" : "enable", rc);
	} else {
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_DISABLE_CHARGE:
		pval->intval = pst->prop[prop_id];
		break;
	case POWER_SUPPLY_PROP_ICHG_LIMIT_STATUS:
		pval->intval = pst->prop[prop_id];
		break;
	default:
		pval->intval = pst->prop[prop_id];
		...
		const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_DISABLE_CHARGE:
		return battery_psy_disable_charge(bcdev, pval->intval);
	case POWER_SUPPLY_PROP_ICHG_LIMIT_STATUS:
		return battery_psy_set_ichg_limit_status(bcdev, pval->intval);
	default:
		return -EINVAL;
	}
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_ICHG_LIMIT_STATUS,
};
