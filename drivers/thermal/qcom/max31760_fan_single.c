...
#define MAX31760_REGISTER_COOLING_DEVICE 0
...
static void max31760_enable_gpio(struct max31760_data *pdata, int on)
{
	...
	if (gpio_is_valid(pdata->pwr_en_gpio)){
		if(gpio_direction_output(pdata->pwr_en_gpio, on)){
			dev_err(pdata->dev, "pwr_en_gpio:%d set %d failed\n", pdata->driver_en_gpio, on);
		}		
	}
	else{
		dev_err(pdata->dev, "pwr_en_gpio:%d is invalid\n", pdata->driver_en_gpio);
	}

	if (gpio_is_valid(pdata->driver_en_gpio)){
		if(gpio_direction_output(pdata->driver_en_gpio, on)){
			dev_err(pdata->dev, "driver_en_gpio:%d set %d failed\n", pdata->driver_en_gpio, on);
		}
	}
	else{
		dev_err(pdata->dev, "driver_en_gpio:%d is invalid\n", pdata->driver_en_gpio);
	}
				...
				unsigned long state)
{
	...
	if (!atomic_read(&pdata->in_suspend)){
		...
		dev_err(pdata->dev, "max31760 fan in suspend, ignore set state(%d)\n", state);
	}
}

#if MAX31760_REGISTER_COOLING_DEVICE
				...
				unsigned long state)
{
	...
	dev_err(pdata->dev, "max31760_set_cur_state set state(%d)\n", state);
	mutex_lock(&data->update_lock);
...
static ssize_t speed_tc_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct max31760_data *data = dev_get_drvdata(dev);
	u32 speed;
	...
	if(data->cur_state == FAN_SPEED_LEVEL0 || atomic_read(&data->in_suspend)){
		speed = FAN_SPEED_LEVEL0;
	}else{
...
static ssize_t power_control_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct max31760_data *data = dev_get_drvdata(dev);
	int ret;
	...
	ret = scnprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&data->in_suspend) ? 0 : 1);
				...
				const char *buf, size_t count)
{
	...
	if(value == 0){
	 	dev_err(dev, "suspend\n");
		mutex_lock(&data->update_lock);
		max31760_enable_gpio(data, 0);
		mutex_unlock(&data->update_lock);
		atomic_set(&data->in_suspend, 1);
	}else{
	 	dev_err(dev, "resume\n");
		mutex_lock(&data->update_lock);
		max31760_enable_gpio(data, 1);
		usleep_range(20000, 20100);
		max31760_write_byte(data, MAX31760_CTRL_REG1, 0x19);
		max31760_write_byte(data, MAX31760_CTRL_REG2, 0x11);
		max31760_write_byte(data, MAX31760_CTRL_REG3, 0x31);
		mutex_unlock(&data->update_lock);
...
static DEVICE_ATTR_RO(speed_tc);
...
static int max31760_parse_dt(struct max31760_data *pdata)
{
	...
	if (ret) {
		dev_err(pdata->dev, "max31760 enable gpio request failed, ret:%d\n", ret);
		goto error;
	}

	ret = gpio_request(pdata->driver_en_gpio, "driver_en_gpio");
	if (ret) {
		dev_err(pdata->dev, "max31760 drvr enable gpio request failed, ret:%d\n", ret);
		goto error;
	}
...
static int max31760_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev,"max31760_probe enter\n");
	...
	if (!pdata){
		dev_err(&client->dev, "device doesn't devm_kzalloc\n");
		return -ENOMEM;		
	}
	...
	dev_err(&client->dev,"max31760_probe exit\n");

	return ret;

#if MAX31760_REGISTER_COOLING_DEVICE
fail_register_cdev:
	dev_err(&client->dev,"max31760_probe fail_register_cdev\n");
	max31760_remove(client);
	return ret;
#endif
fail_parse_dt:
	dev_err(&client->dev,"max31760_probe fail_parse_dt\n");
